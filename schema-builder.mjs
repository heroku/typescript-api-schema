 ...
 *
 * ```ts
 * public async info(accountFeatureIdentity: string): Promise<AccountFeature> {
 *   const response = await this.heroku.get<AccountFeature>(`/account/features/${accountFeatureIdentity}`, {
 *   headers: {
...
function buildClassMembersFromLinks(entityName, links) {
  ...
  for (const link of links) {
    ...
    // methods are used.
    let params = paramDescriptors
      .map(descriptor => `${descriptor.propName}:${descriptor.typeScriptType}`)
      .join(', ');

    params += payloadType ? `${params && ', '}body: ${payloadType}` : '';
    ...
    const response = await this.heroku.${method.toLowerCase()}<${targetSchemaType}>(\`${parameterizedUrl}\`, ${options});
    return response.body;
    }`;
    members.push(member);
  }
...
function createServiceClass(propName, schemaInfo) {
  ...
  const clazz = `import type {APIClient} from '@heroku-cli/command';
