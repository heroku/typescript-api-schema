...
export default class TeamDailyUsageService {
   ...
   */
  public async info(teamId: string, body: Heroku.TeamDailyUsageInfoPayload): Promise<Heroku.TeamDailyUsage[]> {
    const response = await this.heroku.get<Heroku.TeamDailyUsage[]>(`/teams/${teamId}/usage/daily`, {
      body,
      headers: {
