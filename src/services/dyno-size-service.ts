...
export default class DynoSizeService {
   ...
   */
  public async info(dynoSizeIdentity: string): Promise<Heroku.DynoSize> {
    const response = await this.heroku.get<Heroku.DynoSize>(`/dyno-sizes/${dynoSizeIdentity}`, {
      headers: {
   ...
   *
   */
  public async list(): Promise<Heroku.DynoSize[]> {
    const response = await this.heroku.get<Heroku.DynoSize[]>(`/dyno-sizes`, {
      headers: {
        Accept: 'application/vnd.heroku+json; version=3'
      }
    });
