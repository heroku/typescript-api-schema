...
export default class TeamMonthlyUsageService {
   ...
   */
  public async info(teamId: string, body: Heroku.TeamMonthlyUsageInfoPayload): Promise<Heroku.TeamMonthlyUsage[]> {
    const response = await this.heroku.get<Heroku.TeamMonthlyUsage[]>(`/teams/${teamId}/usage/monthly`, {
      body,
      headers: {
