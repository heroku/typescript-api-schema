...
export default class AppWebhookEventService {
  ...
  public async info(
    ...
    const response = await this.fetchImpl(`/apps/${appIdentity}/webhook-events/${appWebhookEventIdentity}`, {
      ...requestInit,
  ...
  public async list(
    ...
    const response = await this.fetchImpl(`/apps/${appIdentity}/webhook-events`, {
      ...requestInit,
