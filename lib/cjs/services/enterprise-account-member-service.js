...
class EnterpriseAccountMemberService {
    ...
    async list(enterpriseAccountIdentity, requestInit = {}) {
        const response = await this.fetchImpl(`/enterprise-accounts/${enterpriseAccountIdentity}/members`, {
            ...requestInit,
            method: 'GET',
            headers: {
                ...requestInit?.headers,
    ...
    async update(enterpriseAccountIdentity, enterpriseAccountMemberUserIdentity, payload, requestInit = {}) {
        const response = await this.fetchImpl(`/enterprise-accounts/${enterpriseAccountIdentity}/members/${enterpriseAccountMemberUserIdentity}`, {
            ...requestInit,
            body: JSON.stringify(payload, null, 2),
            method: 'PATCH',
            headers: {
